# -*- coding: utf-8 -*-
"""
빈 게임 — 사건을 하나 새로 쓸 때 복사해 가는 틀.

이 파일은 «돌아가는 최소한»입니다. 이대로 서버를 켜면 게임이 처음부터 끝까지
...
| `CRISIS` | 게임 중간에 끼는 비상 미니게임 (침수·정전 같은 것) |
...
COMMON_INTRO = (
    ...
    "언제, 어디서, 누가 죽었는지까지만 적습니다 — 왜 죽었는지는 게임이 알아냅니다.\n\n"
...
# ── 알리바이 대화록 ───────────────────────────────────────────────
# 게임이 시작될 때 대화창에 한 줄씩 흘러 들어갑니다(0.5~1.9초 간격, 줄 길이에 따라).
...
ALIBI_LOG = [
    ...
    # note 는 말이 아니라 «게임이 적는 줄»입니다. 대화록 중간을 끊고 상황을 적습니다.
...
PHASES = [
    ...
    {"seq": 8, "key": "reveal", "name": "진상 공개", "round": 3, "min": 12, "ap": 0,
     "gm": "진상과 시각표를 낭독하고 게임을 닫습니다."},
]

# 막이 열릴 때 울리는 방송. 키는 seq. 안 쓰면 빈 dict 로 둡니다.
INTERLUDES: dict = {}

CULPRIT_ID = "gamma"                 # 진범의 배역 id. 없는 게임이면 "" 로 둡니다
...
#   auto     True 면 뒤져서 못 엽니다 — 게임이 때가 되면 엽니다
...
# 게임이 도는 동안 배역에게 떠오르는 기억. 키는 배역 id, 각 항목의 seq 는 페이즈.
MEMORY = {
...
def public_scenario() -> dict:
    ...
    return {
        ...
        # 화면의 이름표. 사건마다 시간대가 달라서 「오늘 밤」이 안 맞는 게임이 있습니다.
...
def compute_ending(votes: dict):
    ...
    `votes` 는 `{지목한 배역id: 지목당한 배역id}` 다. 사람 셋이 하는 게임이라
