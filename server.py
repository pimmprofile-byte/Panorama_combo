...
def fresh_room(sid: str = "") -> dict:
    return {
        ...
        # ── 지금 이 방에 «와 있는» 기기 ────────────────────────────────
        # clientId -> 마지막으로 소식을 물은 시각. «자리를 잡았는가»와는 다른 물음이다.
        # ★ 배역 카드를 가리는 판이 걷히는 조건이 여기 걸린다. 한때 그 조건이
        #   «자리가 다 찼는가»였는데, 자리를 잡는 유일한 길이 그 판에 가려 있어서
        #   아무도 앉을 수 없었다 — 판이 자기를 걷어 줄 손을 자기가 막고 있었다.
        #   «와 있는가»는 화면을 열어 두기만 해도 참이 되므로 그런 고리가 안 생긴다.
        "here": {},
...
def state(clientId: str = "", gm: int = 0, roleId: str = "", key: str = ""):
    ...
    with LOCK:
        ...
        # ── 몇 명이 와 있는가 ──────────────────────────────────────────
        # 소식을 물었다는 것이 곧 «화면을 열어 두고 있다»는 것이다. 자리를 잡기
        # 전에도 참이라, 배역 카드를 가리는 판은 이 수를 보고 걷힌다.
        if clientId:
            ROOM.setdefault("here", {})[clientId] = now
        for cid, t in list((ROOM.get("here") or {}).items()):
            if now - t > 20:           # gmSeats 와 같은 잣대 — 폴링 1.5초의 열 배 남짓
                ROOM["here"].pop(cid, None)
        _hn = len(ROOM.get("here") or {})
        _ho = len(ROOM["roles"])
        st["here"] = {"n": _hn, "of": _ho, "done": _hn >= _ho}
        # ── 「나」를 여기서 한 번만 정한다 ──────────────────────────────
