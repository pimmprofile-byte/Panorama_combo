...
**사람 셋이서 하는 온라인 머더미스터리 엔진.** 패키지 이름은 **GAME DAY** 이고,
지금 두 게임이 들어 있습니다 — 《RULE THE DAY : 데이의 규칙》(제작 중)과
《COMBO》(포스터만 걸린 잠긴 게임).
...
`scenarios/template.py` 는 목록에 안 올라오는 **복사해 가는 틀**입니다.
...
화면 글꼴은 **Neo둥근모**(`assets/fonts/neodgm.woff2`)입니다 — DOS 시절 비트맵
글꼴 「둥근모꼴」을 트루타입으로 옮긴 것이고, SIL Open Font License 1.1 입니다
(`assets/fonts/neodgm-OFL.txt`). 비트맵이라 안티에일리어싱을 끄고 씁니다.
...
scenarios/
  ...
  template.py              빈 게임 — 복사해 가는 틀
docs/시나리오_인터페이스.md  무엇을 채워야 하는가
docs/카드_부르는_법.md      위치 · 뒷면이름 · 카드이름 — 카드를 가리키는 말
