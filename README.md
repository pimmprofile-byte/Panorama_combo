...
**AI가 빈 좌석을 배역으로 대신 플레이하는 온라인 머더미스터리 엔진.**

두 명이서도 네 명짜리 사건을 굴릴 수 있습니다. 각자 자기 기기로 접속해 자기
배역과 자기 정보만 보고, 남은 배역은 AI가 대본대로 채웁니다 — 거짓말하고,
의심하고, 투표까지 합니다. **어느 자리가 AI인지는 늘 화면에 표시됩니다.**
그걸 감추는 건 몰입이 아니라 기망이라서, 이 엔진은 그 표시를 지우지 않습니다.
...
AI 배역을 쓰려면 `ANTHROPIC_API_KEY` 가 필요합니다. 없어도 사람만으로 판은
돕니다 — AI 좌석이 말을 안 할 뿐입니다. `.env.example` 을 참고하세요.
...
`render.yaml` 이 블루프린트입니다. 배포한 뒤 대시보드에서
`ANTHROPIC_API_KEY` 만 넣으면 됩니다.
